import 'package:flutter/material.dart';

const appBgColor = Color(0xFFF5F6FA);
const appBarColor = Color(0xFFF5F6FA);
const appFooterColor = Color(0xFFF5F6FA);
const instaColor = Color(0xFF262626);
const instaBtnColor = Color(0xFF2E384D);
const textFieldBackground = Color(0xFF9E9696);
const feedBorderColor = Colors.white;
const bgStoryColors = [
  const Color(0xffA844A1),
  const Color(0xffAB429A),
  const Color(0xffB43C88),
  const Color(0xffC33269),
  const Color(0xffD7243F),
  const Color(0xffF9A326),
  const Color(0xffF9DD26),
];
const picsBorderColors = [Color(0xFFFC0B7B), Color(0xFF7820AD)];

const white = Color(0xFFFFFFFF);
const grey = Color(0xFFE9EFFA);
const greyish = Color(0xFF4F5055);
const black = Color(0xFF000000);
const blue = Color(0xFF2196F3);

const success = Color(0xff44c93a);
const danger = Color(0xffff4657);
const info = Color(0xff5bc0de);
const warning = Color(0xfffeba06);
